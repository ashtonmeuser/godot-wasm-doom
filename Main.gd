...
var image = Image.new()
var keys = { KEY_ENTER: 13, KEY_BACKSPACE: 127, KEY_SPACE: 32, KEY_LEFT: 0xac, KEY_RIGHT: 0xae, KEY_UP: 0xad, KEY_DOWN: 0xaf, KEY_CTRL: 0x80+0x1d, KEY_ALT: 0x80+0x38, KEY_ESCAPE: 27, KEY_TAB: 9, KEY_SHIFT: 16, KEY_Y: KEY_Y + 32 }
...
func _input(event):
	if event is InputEventKey: wasm.function("add_browser_event", [int(!event.is_pressed()), keys.get(event.keycode, 0)])
