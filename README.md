...
Using a [WebAssembly Doom port](https://diekmann.github.io/wasm-fizzbuzz/doom/) and the [Godot Wasm addon](https://github.com/ashtonmeuser/godot-wasm), the 1993 classic Doom is able to be run and rendered in the [Godot game engine](https://godotengine.org/).
...
Obviously, the title of this article is somewhat misleading. The entirety of Doom can not be implemented in a few dozen lines of GDScript. Instead, we'll be running and rendering a precompiled WebAssembly port of Doom within Godot.
...
To take advantage of Wasm's benefits, I created the [Godot Wasm addon](https://github.com/ashtonmeuser/godot-wasm) for the Godot game engine. This addon enables compiling & initializing Wasm modules, accessing exported functions/globals from Godot, providing imports implemented in GDScript, and accessing Wasm module memory.

Throughout the creation of this project, one question kept nagging at me: **Can it run Doom?**

## Acknowledgement

This project owes a huge thanks to the incredible work of [Cornelius Diekmann](https://github.com/diekmann). Their [WebAssembly from Scratch](https://github.com/diekmann/wasm-fizzbuzz) project provided great insight into the porting process, the underlying Doom Wasm module, and clear guidance on running the module. Diekmann's [Wasm Doom example](https://diekmann.github.io/wasm-fizzbuzz/doom/) will be referenced throughout this writeup.
...
We can get a better idea of the API exposed by the *doom.wasm* module by inspecting it. In the terminal, a similar inspection can be performed with the [Wasmer CLI](https://docs.wasmer.io/install) via `wasmer inspect doom.wasm`.
...
That's a lot of information! Fret not, we'll be ignoring most of this. For now, take note of `import_functions` and `memory` properties.
...
Referring back to the inspection of the module, we can see five import functions. If you inspected the module via the Wasmer CLI tool, you'll also note that the module requires a memory import.
...
Each import function signature is represented by an array of two arrays in the object returned by `wasm.inspect()`. The first array represents the parameter types and the second represents return values. Empty arrays represent no function parameters and a `void` return type, respectively. The types use Godot's [`Variant.Type`](https://docs.godotengine.org/en/stable/classes/class_%40globalscope.html#enum-globalscope-variant-type) enumeration. As an example, the `js.js_console_log` import function takes two integers as arguments and returns no value.

First, we'll satisfy function imports with stubbed out functions. In your main GDScript file, add the following functions.
...
Let's provide these functions to the module as imports during instantiation. Each import function is represented by an array containing a Godot `Object` and the name of the method to call. Once again in `_ready()`, add the following.
...
Create another `@onready` variable in the top level of your main script.
...
When calling our `main()` export function, we could see that the `console_log` import function was called by the Wasm module twice before invocation failed. We'll implement some basic logging to aid in debugging.

Some background regarding Wasm memory is important at this stage of the journey. WebAssembly memory is simply a contiguous buffer or array of bytes. As we saw earlier, this array can be expanded or grown. Memory is very important and frequently used with Wasm because of the limited API that can be exposed via import/export functions, also known as Foreign Function Interface or FFI. Only the following four fundamental data types can be directly exposed via Wasm import/export functions.
- 32-bit integer
- 64-bit integer
- 32-bit floating point
- 64-bit floating point

This begs the question: how do we transfer a string between Godot (host) and the Wasm module (guest)?

The answer is to take advantage of the module's memory. The Wasm module can write a string to memory in an agreed-upon format which can later be read by the host i.e. Godot. The host can be instructed where in memory to begin reading and how many bytes to read with simple integer values passed via import/export functions.

Godot Wasm's `WasmMemory` class (see [WasmMemory class documentation](https://github.com/ashtonmeuser/godot-wasm/wiki/Class-Documentation:-WasmMemory)) inherits from Godot's own [```` class](https://docs.godotengine.org/en/stable/classes/class_`.`html) and closely mirrors Godot's [``Buffer`` class](https://docs.godotengine.org/en/stable/classes/class_`buffer`.html#class-`buffer`). This allows us to easily read raw bytes in a variety of contexts.

We now have the context required to implement our logging functions. We've already seen that the `js.js_console_log` import function accepts two integers as arguments. These integers represent the data offset i.e. starting point and data length, respectively. We'll use these to read the data to be printed. Referring to Diekmann's example, we expect strings to be stored as UTF-8. Reimplement the `stdout` and `console_log` GDScript functions as follows.
...
The `seek()` method moves the cursor to a memory offset while `get_utf8_string()` (inherited from ```)` reads a UTF-8 string from raw bytes.
...
> [!NOTE]
> The macros used in Godot Wasm are currently undergoing changes. Instead of `PRINT_ERROR` you may need to use `GODOT_WASM_PRINT_ERROR` or directly call `UtilityFunctions::print(String(message.data))` depended on the version of Godot Wasm used.
...
Let's try allocating more memory. Using `memory.grow()`, allocate an assortment memory sizes up to the maximum 65536 pages. All values fail with the final, largest value producing the following error.
...
Using Diekmann's example as our guide, this is exactly the output we're expect. Doom is successfully initializing!
...
Let's go ahead and implement a simple error logging function similar to what we did for `console_log()` and `stdout()` to properly satisfy the `js.js_stderr` import function. We'll push error messages printed from Doom to Godot's warning-level logging. This will raise attention to Doom errors while reserving Godot's error-level logging for critical errors encountered while running the Wasm module.
...
Next, let's implement the `js.js_milliseconds_since_start` import function. In the main GDScript file, modify the `milliseconds_since_start()` method to the following.
...
Referring once again to Diekmann's example, we can see that there are only three export functions called.
1. `main`
1. `doom_loop_step`
1. `add_browser_event`

Let's proceed to calling the main Doom game loop. At the end of the `_ready()` function in your GDScript script, call the `doom_loop_step()` export function.
...
The game loop should not just be called once. Rather, it should be called repeatedly, ticking the game along with each invocation. Let's use Godot's `_process()` method call Doom's game loop. Remove the call to `doom_loop_step` in `_ready()` and add the following to your script.
...
Per Diekmann's writeup, there is a custom rendering shim applied on top of classic Doom implemented in Rust. This simplifies our drawing/rendering implementation substantially as it is outputting standard [RGBA8888](https://en.wikipedia.org/wiki/RGBA_color_model#RGBA8888) (also called RGBA8). This format consists of four color channels each with a bit depth of one byte or eight bits. Godot supports this format via the [FORMAT_RGBA8](https://docs.godotengine.org/en/stable/classes/class_image.html#enumerations) flag.
...
As shown by `wasm.inspect()`, the `add_browser_event()` export function accepts two integer arguments and returns `void`. These arguments are as follows.
1. A boolean (represented as an integer) denoting whether a key was released or pressed. This boolean is misleading in that a value of `false` or 0 represents a key pressed while `true` or 1 represents released.
1. An integer key code denoting the key that was pressed or released.
...
This captures many of the required keys but notably missed numeric, alphabetic, and function keys.

Let's modify our `_input()` method to display the mapped keys. All unknown keys will be mapped to key code 0 which is ultimately ignored by Doom. We'll also invert the value of `is_pressed()` and cast it to an integer to match Doom's API.
...
We'll use some array mapping magic to cast each range member to a Dictionary with a single key equal to the Godot key code with a corresponding value equal to the Doom/DOS key code. This format matches that defined by the previously-declared `keys` Dictionary. For each Dictionary, we can then use the [`merge()` method](https://docs.godotengine.org/en/stable/classes/class_dictionary.html#class-dictionary-method-merge) to include them in our mapping. Add the following anywhere in `_ready()`.
...
Done and done. I hope exploration was as entertaining and educational for you as it was for me. Additionally, I hope this convinces you of the incredible power of WebAssembly! As a personal plug, please go star the [Godot Wasm](https://github.com/ashtonmeuser/godot-wasm) project on GitHub. Feel free to open an issue or PR!

The final resultant source code is available [here](https://github.com/ashtonmeuser/godot-wasm-doom). Several of the functions defined above have been altered for brevity, although their logic remains the same.
